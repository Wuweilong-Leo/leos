...
OS_SEC_KERNEL_TEXT void OsMemFscFreeListInsertBlk(struct OsMemFscCtrl *ctrl, struct OsMemFscHead *blk)
{
...
OS_SEC_KERNEL_TEXT struct OsMemFscHead *OsMemFscFuzzySearch(struct OsMemFscCtrl *ptCtrl, size_t size)
{
    // 扫描所有比当前需要的内存大的链表
...
OS_SEC_KERNEL_TEXT bool OsMemFscTrySplitRightBlk(struct OsMemFscCtrl *ctrl, uintptr_t rightBlk, size_t rightBlkSize)
{
...
OS_SEC_KERNEL_TEXT bool OsMemFscTrySplitLeftBlk(struct OsMemFscCtrl *ctrl, uintptr_t leftBlk, size_t leftBlkSize, uintptr_t curBlk)
{
...
OS_SEC_KERNEL_TEXT void *OsMemFscAlloc(struct OsMemFscCtrl *ptCtrl, size_t size, U32 align)
{
    ...
    // 从后往前切割，获得能保证对齐的首地址
    realBlk = OS_ROUND_DOWN(nextBlk - OS_MEM_FSC_TAIL_MAGIC_SIZE - alignSize, align) - OS_MEM_FSC_HEAD_SIZE;
...
OS_SEC_KERNEL_TEXT void OsMemFscTryMergeRight(struct OsMemFscCtrl *ctrl, struct OsMemFscHead *curBlk)
{
